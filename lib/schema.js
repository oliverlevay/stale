...
const fields = {
  ...
  daysUntilApprove: Joi.alternatives().try(Joi.number(), Joi.boolean().only(false))
    .error(() => '"daysUntilApprove" must be a number or false')
  ...
  approveComment: Joi.alternatives().try(Joi.string(), Joi.boolean().only(false))
    .error(() => '"approveComment" must be a string or false')
...
const schema = Joi.object().keys({
  daysUntilStale: fields.daysUntilStale.default(3),
  daysUntilApprove: fields.daysUntilApprove.default(1),
  ...
  approveComment: fields.approveComment.default(false),
